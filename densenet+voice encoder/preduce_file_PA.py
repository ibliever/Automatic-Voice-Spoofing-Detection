...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        path = 'extract_pre/PA/spect_sedense/train/'+batch_meta.file_name[0]+'.pkl'
...
if __name__ == '__main__':
    ...
    dataset = data_utils.ASVDataset(is_train=True, is_logical=False,
                                    transform=transforms,
                                    feature_name='spect_sedense', is_eval=False, eval_part=0)

    model_cls = SEDenseNet
    model = model_cls().to(device)
    model = nn.DataParallel(model, device_ids=[0, 1, 2, 3])
    model.load_state_dict(torch.load('./models/model_physical_spect_sedense_200_32_5e-05/epoch_198.pth'))
