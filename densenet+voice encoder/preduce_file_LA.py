...
import models
from models import FeatureExtractor, DenseNet, seDenseTransNet
import data_utils
import numpy as np
...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        path = 'extract_pre/sedensetrans/eval/'+batch_meta.file_name[0]+'.pkl'
...
if __name__ == '__main__':
    ...
    dataset = data_utils.ASVDataset(is_train=False, is_logical=True,
                                    transform=transforms,
                                    feature_name='lfcc_sedensetrans', is_eval=True, eval_part=0)

    model_cls = seDenseTransNet
    model = model_cls().to(device)
    model = nn.DataParallel(model, device_ids=[0, 1, 2, 3])
    model.load_state_dict(torch.load('./models/model_logical_lfcc_sedensetrans_200_32_5e-05/epoch_60.pth'))
