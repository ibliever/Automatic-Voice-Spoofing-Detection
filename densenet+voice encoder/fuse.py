...
if __name__ == '__main__':
    ...
    train_audio_path = '/media/data3/zhou/data'
    dev_audio_path = '/media/data3/zhou/data'
    eval_audio_path = '/media/data3/zhou/data'

    for file in train_file:
        face = pkl.load(open('extract_pre/face/train/'+file+'.pkl', 'rb'))
        audio = pkl.load(open('/media/data3/zhou/data/'+file+'.pkl', 'rb'))
        audio = np.reshape(audio, newshape=(1,256))
        fuse = np.concatenate([face, audio], axis=1)
        path = 'extract_pre/PA/fuse_cqt/' + file + '.pkl'
