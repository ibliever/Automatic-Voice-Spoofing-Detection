...
class ASVDataset(Dataset):
    def __init__(self, data_type=None):
        ...
        if self.data_type=='dev':
            self.cache_fname = 'cache_fuse_sedensetrans_dev.npy'
            self.protocols_fname = 'ASVspoof2019.LA.cm.dev.trl.txt'
            self.files_dir = './extract_pre/fuse_sedensetrans/dev'
        if self.data_type=='train':
            self.cache_fname = 'cache_fuse_sedensetrans_train.npy'
            self.protocols_fname = 'ASVspoof2019.LA.cm.train.trn.txt'
            self.files_dir = './extract_pre/fuse_sedensetrans/train'
        if self.data_type=='eval':
            self.cache_fname = 'cache_fuse_sedensetrans_eval.npy'
            self.protocols_fname = 'ASVspoof2019.LA.cm.eval.trl.txt'
            self.files_dir = './extract_pre/fuse_sedensetrans/eval'
        self.sysid_dict = {
            '-': 0,  # bonafide speech
